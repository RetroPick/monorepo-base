...
test.describe("J07 — order preview, sign, submit", () => {
  test("e2e-j07 happy path with kill switch on submits order", async ({ page }) => {
    const submitCalls: unknown[] = [];
    await setupMarketsE2EPage(page, { orderSubmit: true, captureSubmit: { calls: submitCalls } });
    ...
    await expect(dialog.getByRole("heading", { name: /review order before signing/i })).toBeVisible();
  ...
  test("e2e-j07 kill switch off does not submit after sign", async ({ page }) => {
    const submitCalls: unknown[] = [];
    await setupMarketsE2EPage(page, { orderSubmit: false, captureSubmit: { calls: submitCalls } });
    await gotoMarketDetail(page);
