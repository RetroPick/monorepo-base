...
test.describe("J03 — market and rules review", () => {
  test("e2e-j03 loads market detail with rules and order ticket", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("region", { name: /resolution rules/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: "Official results" }),
    ).toBeVisible();
    await expect(
      page.getByRole("region", { name: /order ticket/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("region", { name: /order book snapshot/i }),
    ).toBeVisible();
  });

  test("e2e-j03 blocked eligibility disables preview flow", async ({
    page,
  }) => {
    await setupMarketsE2EPage(page, {
      eligible: false,
      eligibilityReason: "geo_denied",
    });
    await gotoMarketDetail(page);
