...
│
├── contracts/                    # renamed from package/prediction-v2 (standard name)
│   ├── src/
...
| **Rename** | `apps/backend` → `services/backend`, `package/prediction-v2` → `contracts`, `fe-v1` → `web` | Standard names, less cognitive load |
...
24. `→ apps/ops/src/features/reporter-queue/` — review UI, conflict flags, evidence panel.
