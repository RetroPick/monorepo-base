...
### 0.2 Optional: yield router

1. Deploy [`YieldRouterV2`](../src/yield/YieldRouterV2.sol). It preserves the [`IYieldRouterV2`](../src/interfaces/IYieldRouterV2.sol) accounting surface and reports API version `2` to expose the production route-policy surface in [`IYieldRouterV3`](../src/interfaces/IYieldRouterV3.sol).
2. **`admin`** on the engine: `setYieldRouter(router, yieldFeeBps)`. Clearing the router zeroes `yieldFeeBps` and disables LM claims.
3. **Router owner** (separate from engine admin): configure a route with `setYieldRoute(routeId, kind, strategy, enabled, cap)` when using a non-default vault, then bind each template with `setTemplateYieldRoute(templateId, routeId)` before `initializeMarket`.
4. **`initializeMarket(templateId)`** calls `lockTemplateYieldRoute` when the router API version is `>= 2`. If the route is missing or disabled, initialization reverts with `InvalidYieldRoute(templateId)`.

Built-in route IDs:

- `ROUTE_AAVE_ATOKEN`: Aave V3 aToken path through the immutable pool.
- `ROUTE_AAVE_STATA`: ERC-4626 Stata/vault path when configured at router deploy time.

Additional ERC-4626 vault routes are supported when `IERC4626(strategy).asset() == STAKE_TOKEN`. Morpho, Yearn, Balancer, or other yield sources should be integrated behind audited ERC-4626 strategy wrappers first; the engine locks only the route ID and strategy address, not provider-specific internals.
...
- `marketType`: canonical enum only: `Direction`, `Threshold`, `RangeClose`, `Velocity`, `Ladder`, `Convergence`, `Composite`, `Corridor`, `Cascade`. Historical variants (`Anchor`, `VolatilityBand`, `StakingAPR`, `BitcoinIRC`, `NAVThreshold`, `MacroEvent`) are product aliases mapped onto these canonical enum values plus `oracleClass` / threshold fields.
...
- `switchFeeBps`, `settlementFeeBps`
  - `settlementFeeBps` is capped by `MAX_PROTOCOL_FEE_BPS = 200` (2.00%) and is charged only from the original losing stake pool at resolution. No protocol fee is charged on refunds / no-winner outcomes.
  - `switchFeeBps` is capped by the deployment `maxSwitchFeeBps`, which itself cannot exceed `MAX_PROTOCOL_FEE_BPS`. Ops defaults it to `0` for launch.
  - Fees are snapshotted into each epoch when the epoch opens, so later template edits do not mutate live or historical epoch economics.
...
- **Chainlink scalar templates are rejected:** `upsertTemplate` reverts for `Corridor + Chainlink` because the scalar Chainlink path does not populate OHLC fields. Use `TrustedReporter` + `getOhlcResult` until a dedicated Chainlink OHLC/history adapter exists.
- **Rolling:** **not allowed**.

### 4.10 Cascade

- Same OHLC / TrustedReporter requirement as **Corridor**. `Cascade + Chainlink` scalar templates are rejected at upsert.
...
| Corridor | **No** (`InvalidTemplate`; Chainlink scalar path has no OHLC) | **Intended** (TRO + `getOhlcResult`) | **No** | **No** |
| Cascade | **No** (`InvalidTemplate`; same OHLC requirement) | **Intended** | **No** | **No** |
...
- Settlement fee is computed from the original losing stake pool only. Routed net yield, when present, is added to the claim liability after fee calculation and distributed pro-rata to winners (see §13.5).
- Reserves: `claimLiabilityTotal` and `settlementFeeTotal` move from active → claims / fee reserves; epoch becomes `claimable`.
- `ProtocolFeeAccrued(templateId, epochId, feeAsset, feeAmount, feeBps, losingPool, winningPool)` is emitted when a non-zero settlement fee is reserved.
...
From the current focused gas suite:
...
| `claim` | 50068 |
| `genesisStartRolling` | 196771 |
| `genesisLockRolling` | 257393 |
| `executeRollingRound` (steady) | 441723 |
...
- Manual Direction per epoch \(\approx 193209 + 128938 + 209039 = 531186\) gas.
- Rolling Direction steady per interval \(\approx 441723\) gas, but **1 tx instead of 3**.
...
## 13.5) Yield routing (`IYieldRouterV2` accounting + `IYieldRouterV3` route policy)
...
- **Route lock**: when the configured router reports `yieldRouterApiVersion() >= 2`, `initializeMarket(templateId)` calls `IYieldRouterV3.lockTemplateYieldRoute(templateId)`. A missing, disabled, or invalid route reverts `InvalidYieldRoute(templateId)`. Older V2-compatible mocks/routers are not force-locked.
...
- **Preferred**: [`YieldRouterV2`](../src/yield/YieldRouterV2.sol) — scaled aToken accounting per `templateId`, Aave V3 aToken route, ERC-4626 route support, route caps, reserve health checks, and `claimLmRewards` for liquidity-mining sweeps. It implements [`IYieldRouterV3`](../src/interfaces/IYieldRouterV3.sol) while keeping all V2 accounting calls.
...
- `ROUTE_AAVE_ATOKEN`: immutable Aave V3 pool route.
- `ROUTE_AAVE_STATA`: deploy-time ERC-4626 Stata/vault route when configured.
- custom routes: router owner calls `setYieldRoute(routeId, ERC4626, strategy, enabled, cap)`; the ERC-4626 vault must use the stake token as `asset()`.
- template binding: router owner calls `setTemplateYieldRoute(templateId, routeId)` before `initializeMarket`; initialization locks the route.
...
- **Tests**: harness in [`test/MarketEngineBase.t.sol`](../test/MarketEngineBase.t.sol); mocks often under [`src/mocks/`](../src/mocks/) (e.g. `MockPriceOracle`, `MockAavePool`).
...
- [`rollingMarket.md`](../rollingMarket.md) — rolling execution mode (canonical).
