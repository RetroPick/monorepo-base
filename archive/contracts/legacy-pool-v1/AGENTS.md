...
- Canonical `TokenFaucet` JSON ABI for integrators: `packages/legacy/abi/TokenFaucet.json` (and backend copy); keep `nonces` / `requestWithSig` aligned with `TokenFaucet.sol` after contract changes.
