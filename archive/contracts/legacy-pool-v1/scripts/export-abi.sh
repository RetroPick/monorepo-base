...
emit_abi() {
  ...
  local contract="${spec##*:}"
  local source="${spec%%:*}"
  local artifact="${ROOT}/out/${source}/${contract}.json"
  if [[ ! -f "$artifact" ]]; then
    artifact="${ROOT}/out/$(basename "$source")/${contract}.json"
  fi
  if [[ ! -f "$artifact" ]]; then
    echo "missing artifact: $artifact" >&2
    exit 1
  fi
  jq '.abi' "$artifact" > "${ABI_DIR}/${out}.json"
...
emit_abi "src/mocks/faucet/TokenFaucet.sol:TokenFaucet" "TokenFaucet"
emit_abi "src/mocks/MockERC20.sol:MockERC20" "MockERC20"
