...
Ship a **single operator workflow** doc and surface it in **`apps/ops-web`** so admin/market/deploy work is sequenced; speed up ops bundle via Next `optimizePackageImports`.
...
- [x] `apps/ops-web/next.config.mjs` enables `experimental.optimizePackageImports` for lucide/recharts/radix.
