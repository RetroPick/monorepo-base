...
contract MarketEngine is Initializable, ReentrancyGuardTransient, UUPSUpgradeable {
    ...
    /// @dev Pancake-style onchain participation index for UI pagination.
    ...
    function genesisLockRolling(bytes32 templateId) external onlyWorkerOrAdmin notPausedWorkerOps nonReentrant {
        ...
        (bool ok, int256 p, uint64 pt, uint256 c, uint80 rid) = _tryReadOracle(templateId, t.oracleFeedId, maxDelay, nowTs);
        if (!ok) {
    ...
    /// @notice Pancake-style pagination for a user's participated epochs within a template.
