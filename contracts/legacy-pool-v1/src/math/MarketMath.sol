...
library MarketMath {
    ...
    /// @dev Distributable winner upside for pro-rata claims. Once resolved, prefer `claimLiabilityTotal`
    /// because it includes any net yield already added to the claim reserve.
    function _distributableLosingPoolForClaims(
        ...
        if (epoch.claimLiabilityTotal != 0) {
            if (epoch.claimLiabilityTotal < winningPool) revert MathOverflow();
            return epoch.claimLiabilityTotal - winningPool;
        }
