...
contract MarketEngineViewModule is MarketEngineState {
        ...
        returns (IMarketEngine.TemplateYieldView memory view_)
    {
        ...
        try IYieldRouterV3(address(r)).getTemplateYieldRoute(templateId) returns (
            IYieldRouterV3.TemplateYieldRouteView memory route
        ) {
            view_.yieldRouteId = route.routeId;
            view_.yieldStrategy = route.strategy;
            view_.yieldStrategyKind = uint8(route.kind);
            view_.yieldRouteEnabled = route.enabled;
            view_.yieldRouteLocked = route.locked;
            view_.yieldRouteCap = route.cap;
        } catch {}
    }
