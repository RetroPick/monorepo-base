...
contract MarketEngineUserOpsClaimsModule is MarketEngineState, ReentrancyGuardTransient {
        ...
        nonReentrant
    {
        ...
        if (pos.stakes[fromOutcome] == 0) {
            pos.occupiedMask &= ~_outcomeBit(fromOutcome);
        }
        pos.occupiedMask |= _outcomeBit(toOutcome);
    ...
    function _depositToSide(
        ...
        pos.occupiedMask |= _outcomeBit(outcomeIndex);
    ...
    function _canDepositToOutcome(
        ...
        return (pos.occupiedMask & ~_outcomeBit(outcomeIndex)) == 0;
    }
        ...
        returns (bool)
    {
        ...
        uint8 outcomeBit = _outcomeBit(outcomeIndex);
        if ((mask & outcomeBit) == 0) return false;
        return (mask & ~outcomeBit) == 0;
    }
    ...
    function _outcomeBit(uint8 outcomeIndex) internal pure returns (uint8) {
        // outcomeIndex is bounded by callers to < MarketTypes.MAX_OUTCOMES.
        return uint8(uint256(1) << uint256(outcomeIndex));
    }
