//! Integration-style **math and accounting** regressions for claim rounding, switch fees, and ledger moves.
//! Not a full local-validator integration test; exercises pure functions and small struct fixtures.
...
#[test]
fn final_winner_sweeps_only_epoch_scoped_dust_not_global_reserve() {
    // SECURITY: claim.rs passes epoch_remaining = claim_liability_total - claimed_total as the
    // dust-sweep ceiling, NOT ledger.claims_reserve_total (which spans all pending epochs).
    let mut epoch = sample_epoch(100, 200);
    epoch.status = EpochStatus::Resolved;
    epoch.winning_outcome_mask = 1;
    epoch.total_pool = 10;
    ...
    epoch.claim_liability_total = 10;
    epoch.claimed_total = 6; // two earlier winners claimed 3 each
    ...
    // epoch_remaining = 10 - 6 = 4; a buggy caller would pass ledger.claims_reserve_total
    // which could be e.g. 504 (epoch-N dust 4 + epoch-N+1 pending 500).
    let epoch_remaining = epoch.claim_liability_total.saturating_sub(epoch.claimed_total);
    let (payout, winning_stake) = compute_claim_payout(&epoch, &position, epoch_remaining).unwrap();
    assert_eq!(winning_stake, 1);
    assert_eq!(payout, 4);
}

#[test]
fn cross_epoch_inflation_does_not_bleed_into_earlier_epoch_dust_sweep() {
    let mut epoch = sample_epoch(100, 200);
    epoch.status = EpochStatus::Resolved;
    epoch.winning_outcome_mask = 1;
    epoch.total_pool = 10;
    ...
    let inflated_global_reserve = 504u64; // epoch N remaining (4) + epoch N+1 pending (500)
    let epoch_remaining = epoch.claim_liability_total.saturating_sub(epoch.claimed_total);

    let (payout_correct, _) = compute_claim_payout(&epoch, &position, epoch_remaining).unwrap();
    let (payout_buggy, _) = compute_claim_payout(&epoch, &position, inflated_global_reserve).unwrap();

    assert_eq!(payout_correct, 4);
    assert_eq!(payout_buggy, 504);
    assert!(payout_correct < payout_buggy);
}

#[test]
fn is_fully_settled_empty_epoch_is_trivially_closeable() {
    let epoch = sample_epoch(100, 200); // total_pool == 0 by default
    assert!(epoch.is_fully_settled());
}

#[test]
fn is_fully_settled_resolved_epoch_with_pending_winners_cannot_close() {
    let mut epoch = sample_epoch(100, 200);
    epoch.claimable = true;
    epoch.total_pool = 100;
    epoch.remaining_winning_stake = 5;
    assert!(!epoch.is_fully_settled());
}

#[test]
fn is_fully_settled_resolved_epoch_all_winners_claimed_can_close() {
    let mut epoch = sample_epoch(100, 200);
    epoch.claimable = true;
    epoch.total_pool = 100;
    epoch.remaining_winning_stake = 0;
    assert!(epoch.is_fully_settled());
}

#[test]
fn is_fully_settled_refund_mode_partial_claims_cannot_close() {
    let mut epoch = sample_epoch(100, 200);
    epoch.claimable = true;
    epoch.refund_mode = true;
    epoch.total_pool = 100;
    epoch.total_refund_liability = 100;
    epoch.claimed_total = 50;
    assert!(!epoch.is_fully_settled());
}

#[test]
fn is_fully_settled_refund_mode_fully_refunded_can_close() {
    let mut epoch = sample_epoch(100, 200);
    epoch.claimable = true;
    epoch.refund_mode = true;
    epoch.total_pool = 100;
    epoch.total_refund_liability = 100;
    epoch.claimed_total = 100;
    assert!(epoch.is_fully_settled());
}
