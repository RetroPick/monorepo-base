//! **Direction** markets: compare checkpoint **A** (lock) vs **B** (`flow.md` §2.1).

use anchor_lang::prelude::*;
use crate::{errors::MarketError, state::OracleCheckpoint};

/// Outcome mask: bit `i` = outcome `i` wins. **Binary:** `1<<0` vs `1<<1`. Returns `None` if B equals A and `void_on_equal` (refund void path).
///
/// Tests: B higher → outcome 0 (`1`); B lower → outcome 1 (`2`); equal + void → `None`.
