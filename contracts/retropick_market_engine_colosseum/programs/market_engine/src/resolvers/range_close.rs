//! **RangeClose** markets: bucket `checkpoint_b.value_e8` against sorted `range_bounds_e8` (`flow.md` §2.2).

use anchor_lang::prelude::*;
use crate::{constants::MAX_OUTCOMES, errors::MarketError, state::OracleCheckpoint};

/// Returns a single-bit `u64` mask for the winning outcome index. First bucket: `value < bounds[0]`; last bucket: `value` ≥ all interior bounds.
///
/// **Example:** `outcome_count = 3`, `bounds = [100, 200, ...]` → `value=50` → outcome 0 (mask `1`); `value=150` → outcome 1 (mask `2`).
