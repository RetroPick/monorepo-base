//! Fee split for [`crate::instructions::market::switch_side`]: gross → net stake + protocol fee.

use anchor_lang::prelude::*;

use crate::{constants::BPS_DENOMINATOR, errors::MarketError};

/// Splits `gross_amount` into `(net_to_destination, fee)` using **ceiling** fee: `ceil(gross * bps / 10_000)` so splitting trades cannot evade fees (`split_switches_do_not_avoid_fees` test).
///
/// **Examples:** `gross=10000, bps=200` → fee **200**, net **9800**. `gross=199, bps=1` → fee **1**, net **198** (rounds up).
