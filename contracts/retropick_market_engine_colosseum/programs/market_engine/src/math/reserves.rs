//! Moves **notional** balances between [`crate::state::MarketLedger`] fields to match SPL flows (active / claims / fee vaults).

use anchor_lang::prelude::*;
use crate::{errors::MarketError, state::MarketLedger};

/// Decreases `active_collateral_total` and increases `claims_reserve_total` by `amount` (collateral earmarked for claims after resolve).
...
/// Decreases `active_collateral_total` and increases `fee_reserve_total`.
...
/// Switch fees are protocol revenue: same as [`reserve_fees_from_active`].
pub fn reserve_switch_fee_from_active(ledger: &mut MarketLedger, amount: u64) -> Result<()> {
    reserve_fees_from_active(ledger, amount)
}
/// When a user claims, decrements `claims_reserve_total` by `amount` (SPL left the claims vault).
...
/// When treasury withdraws fees, decrements `fee_reserve_total`.
