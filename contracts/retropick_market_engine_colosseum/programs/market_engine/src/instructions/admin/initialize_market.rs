//! Per-template **first** bootstrap after [`crate::instructions::admin::upsert_template`]: ledger + three vault metas + three token ATAs.
//!
//! **Checklist:** (1) `[ledger]` PDA. (2) For each vault: `[meta]` PDA + **unchecked** authority PDA + **ATA** with `seeds = [..._token, template]` and authority = vault PDA. (3) `stake_mint` must match [`Config::stake_mint`].
//! **CPIs:** SPL `init` for token accounts via Anchor constraints; no user stake movement.
...
/// **Signer:** `admin` = [`Config::admin`]. Initializes ledger at zero collateral; vault authority bumps stored on metas.
#[derive(Accounts)]
...
/// Validates mint, zeroes ledger and meta structs, emits [`MarketInitialized`]. Idempotent-safe only at first init (accounts are `init`).
pub(crate) fn handler(ctx: Context<InitializeMarket>) -> Result<()> {
