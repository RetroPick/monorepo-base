//! Create or update a [`MarketTemplate`] PDA by slug. Enforces config fee/outcome caps.

use anchor_lang::prelude::*;
use crate::{constants::{MAX_OUTCOMES, VERSION}, errors::MarketError, events::TemplateUpserted, state::*};

/// **Signers:** `payer` (rent if init), `admin` = [`Config::admin`]. `init_if_needed` on template PDA.
#[derive(Accounts)]
...
/// Template fields; `equal_price_voids` / `fee_on_losing_pool` are fixed in handler to protocol defaults today.
...
pub struct UpsertTemplateParams {
    ...
    /// `0` = inherit global [`crate::state::Config::oracle_config`] staleness.
    pub oracle_max_delay_seconds: i64,
    /// `0` = inherit global confidence cap.
    pub oracle_max_confidence_bps: u16,
}

/// Validates caps, slug immutability, [`MarketTemplate::validate`], emits [`TemplateUpserted`]. **No token CPI.**
pub(crate) fn handler(ctx: Context<UpsertTemplate>, params: UpsertTemplateParams) -> Result<()> {
    ...
    template.oracle_max_delay_seconds = params.oracle_max_delay_seconds;
    template.oracle_max_confidence_bps = params.oracle_max_confidence_bps;
    template.reserved = [0; 6];
    template.validate()?;
