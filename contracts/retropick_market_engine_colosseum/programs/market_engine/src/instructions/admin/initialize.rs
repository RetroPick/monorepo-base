//! First-time deployment: creates the singleton [`Config`] PDA. See `docs/current/currentPrograms.md` §3.

use anchor_lang::prelude::*;

use crate::{constants::VERSION, events::ConfigInitialized, state::*};

/// **Accounts:** `payer` funds rent; `admin` becomes [`Config::admin`]. Initializes `[Config::SEED]` account.
#[derive(Accounts)]
...
/// Constructor args for [`Config`]: treasury, worker, stake mint, fee caps, oracle policy.
...
/// Writes all [`Config`] fields, runs [`Config::validate`], emits [`ConfigInitialized`]. **No CPI** beyond `init`.
pub(crate) fn handler(ctx: Context<InitializeConfig>, params: InitializeConfigParams) -> Result<()> {
