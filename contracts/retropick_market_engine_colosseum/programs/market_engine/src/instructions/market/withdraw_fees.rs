//! Pull protocol fees from fee vault to treasury ATA. **Note:** handler does not read [`Config::paused`] (treasury/admin may withdraw anytime fees exist).

use anchor_lang::prelude::*;
use anchor_spl::token_interface::{transfer_checked, Mint, TokenAccount, TokenInterface, TransferChecked};
use crate::{errors::MarketError, events::FeesWithdrawn, math::release_fee_on_withdraw, state::*};

/// **Signer:** `authority` = `admin` OR `treasury`. Treasury must own `treasury_token_account`.
#[derive(Accounts)]
...
/// **Pre:** `amount > 0`, `ledger.fee_reserve_total >= amount`. **CPI:** fee vault PDA → `treasury_token_account`. **Post:** [`release_fee_on_withdraw`], [`FeesWithdrawn`].
pub(crate) fn handler(ctx: Context<WithdrawFees>, amount: u64) -> Result<()> {
