//! User deposits stake SPL into the **active** vault and updates [`Position`] / epoch pools.

use anchor_lang::prelude::*;
use anchor_spl::token_interface::{transfer_checked, Mint, TokenAccount, TokenInterface, TransferChecked};
use crate::{constants::VERSION, errors::MarketError, events::PositionDeposited, state::*};

/// **Signer:** `user`. **Pause:** yes. User ATA → `active_vault` (no PDA signer on user side).
#[derive(Accounts)]
...
/// **Pre:** epoch open for betting, active epoch id, single-side rules. **CPI:** `transfer_checked` user → active vault. **Post:** increases pools and [`MarketLedger::active_collateral_total`].
pub(crate) fn handler(ctx: Context<DepositToSide>, outcome_index: u8, amount: u64) -> Result<()> {
