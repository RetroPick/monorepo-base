//! Opens the next sequential [`Epoch`] after [`MarketLedger::require_can_open_next_epoch`] (`rollin-rounds.md`).

use anchor_lang::prelude::*;
use crate::{constants::VERSION, errors::MarketError, events::EpochOpened, state::*};

/// **Signers:** `payer`, `authority` ∈ {worker, admin}. **Pause:** enforced on config. Template must be `active`.
#[derive(Accounts)]
...
/// Wall-clock gates for the new epoch (`flow.md` §1).
#[derive(AnchorSerialize, AnchorDeserialize, Clone)]
pub struct OpenEpochParams { pub open_at: i64, pub lock_at: i64, pub resolve_at: i64 }

/// **Pre:** strict timing `open < lock < resolve`. **Post:** new epoch `Open`, snapshot template fields, `ledger.active_epoch_id = epoch_id`. **No SPL CPI.**
pub(crate) fn handler(ctx: Context<OpenEpoch>, epoch_id: u64, params: OpenEpochParams) -> Result<()> {
