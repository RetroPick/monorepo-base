//! Refund path: moves entire `total_pool` to claims vault, enables refund claims. **Ignores pause.**

use anchor_lang::prelude::*;
use anchor_spl::token_interface::{transfer_checked, Mint, TokenAccount, TokenInterface, TransferChecked};
use crate::{errors::MarketError, events::EpochCancelled, math::reserve_claims_from_active, state::*};

/// **Signer:** worker/admin. **Pause:** not checked on `config`.
#[derive(Accounts)]
...
/// **Pre:** epoch `Open` or `Locked`, `reason != None`. **CPI:** active → claims for full pool. **Post:** `refund_mode`, `claimable`, `last_resolved_epoch_id`.
pub(crate) fn handler(ctx: Context<CancelEpoch>, reason: CancelReason, voided: bool) -> Result<()> {
