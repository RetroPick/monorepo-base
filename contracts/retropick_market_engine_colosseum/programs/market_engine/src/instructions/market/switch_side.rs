//! Reallocate stake between outcomes; fee skimmed to **fee** vault via active vault PDA signer.
...
/// **Signer:** `user`. Includes active + fee vault PDAs for fee CPI.
#[derive(Accounts)]
...
/// **Pre:** open betting window; optional full-balance switch in single-side mode. **CPI (if fee>0):** active → fee vault, PDA `active_vault_authority`. **Post:** [`reserve_switch_fee_from_active`].
pub(crate) fn handler(ctx: Context<SwitchSide>, from_outcome: u8, to_outcome: u8, gross_amount: u64) -> Result<()> {
