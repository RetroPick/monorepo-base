//! Writes checkpoint **B**, runs resolver, moves SPL active → claims + fee vaults, finalizes epoch (`flow.md`).
...
/// **Signer:** worker/admin. Full vault set + Pyth account for checkpoint B.
#[derive(Accounts)]
...
/// **Pre:** `is_resolvable`, B not written, confidence ok. **CPIs:** active vault authority → claims + fee vaults. **Post:** `claimable`, `last_resolved_epoch_id`, terminal status Resolved or Voided.
pub(crate) fn handler(ctx: Context<ResolveEpoch>) -> Result<()> {
