//! User withdraws from **claims** vault. **Does not read** global pause flag on config (exit path).
...
/// **Signer:** `user`. `config` included for mint check only.
#[derive(Accounts)]
...
/// **Pre:** `epoch.claimable`, not already claimed, positive payout. **CPI:** claims vault PDA → user ATA. **Post:** position claimed, epoch counters, [`release_claim_on_withdraw`].
pub(crate) fn handler(ctx: Context<Claim>) -> Result<()> {
    ...
    // Use the epoch-scoped remaining liability as the dust-sweep ceiling, not the
    // global ledger reserve. The ledger reserve spans all pending epochs for this
    // market; passing it to the last-winner sweep would let them drain funds that
    // belong to claimants from a concurrently-pending epoch.
    let epoch_remaining_claims = ctx.accounts.epoch.claim_liability_total
        .saturating_sub(ctx.accounts.epoch.claimed_total);
