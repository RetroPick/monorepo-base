...
- **Full suite (Rust + local validator + TS):** `npm test` or `NO_DNA=1 anchor test` (starts `solana-test-validator`, deploys the program, runs Mocha per `Anchor.toml`).
- **Rust only:** `cargo test -p market_engine`
- **TS only (needs validator already running on localhost):** `npm run test:single-side` (and other `test:*` scripts in `package.json`).
