...
/** Metal.XAU/USD on Hermes — use with `PYTH_TEST_FEED_ID` for non-crypto integration runs. */
export const EXAMPLE_XAU_USD_FEED_ID =
  "0x765d2ba906dbc32ca17cc11f5310a89e9ee1f6420508c63861f2f8ba4ee34bb2";

/** Hermes / env override for integration tests (`market_lifecycle`, stale, confidence). */
export function getTestFeedId(): string {
  return process.env.PYTH_TEST_FEED_ID ?? DEFAULT_BTC_USD_FEED_ID;
}

/** Display-only asset label for slugs in tests; default `BTC`. */
export function getTestAssetSymbol(): string {
  return process.env.PYTH_TEST_ASSET_SYMBOL ?? "BTC";
}
