...
function loadProgramId(): PublicKey {
  ...
  if (fs.existsSync(anchorTomlPath)) {
    ...
    const localnetMatch = anchorToml.match(/^\s*market_engine\s*=\s*"([^"]+)"/m);
    if (localnetMatch) {
      return new PublicKey(localnetMatch[1]);
    }
  }
...
export type TemplateConfig = {
  ...
  /** When set and non-zero, overrides config oracle staleness for this template (Borsh: i64 + u16 after bool). */
  oracleMaxDelaySeconds?: number;
  oracleMaxConfidenceBps?: number;
};
...
function readAnchorToml(): string | undefined {
  const anchorTomlPath = path.resolve(process.cwd(), "Anchor.toml");
  if (!fs.existsSync(anchorTomlPath)) {
    return undefined;
  }
...
export function providerFromEnv(): anchor.AnchorProvider {
  ...
  const anchorToml = readAnchorToml();
  const walletFromToml = anchorToml?.match(/^\s*wallet\s*=\s*"([^"]+)"/m)?.[1];
  const walletPath = (process.env.ANCHOR_WALLET ?? walletFromToml ?? "~/.config/solana/id.json").replace(
    /^~(?=\/)/,
    os.homedir()
  );
  ...
  const provider = new anchor.AnchorProvider(
    new Connection(providerUrl, "confirmed"),
    wallet,
...
export function upsertTemplateIx(params: {
  ...
  const data = Buffer.concat([
    ...
    i64(params.oracleMaxDelaySeconds ?? 0),
    u16(params.oracleMaxConfidenceBps ?? 0),
  ]);
...
export function withdrawFeesIx(
  ...
  return withdrawFeesIxWithAuthority(ctx, ctx.admin.publicKey, amount);
}

/** Same as [`withdrawFeesIx`] but with an explicit fee-withdrawal signer (must be config admin or treasury). */
export function withdrawFeesIxWithAuthority(
  ctx: Pick<
    TestContext,
    | "config"
    | "template"
...
export function closeEpochIxWithAuthority(
  ctx: Pick<TestContext, "config" | "template" | "epoch">,
  authority: PublicKey,
  receiver: PublicKey
): TransactionInstruction {
  return new TransactionInstruction({
    ...
    data: discriminator("close_epoch"),
  });
}

export function closeEpochIx(
  ctx: Pick<TestContext, "admin" | "config" | "template" | "epoch">,
  receiver: PublicKey
): TransactionInstruction {
  return closeEpochIxWithAuthority(ctx, ctx.admin.publicKey, receiver);
}

/** `deposit_to_side` with optional account substitution for adversarial tests. */
export function depositToSideIxWithOverrides(
  ctx: TestContext,
  outcomeIndex: number,
  amount: bigint,
  overrides: {
  ...
  const data = Buffer.concat([discriminator("deposit_to_side"), Buffer.from([outcomeIndex]), u64(amount)]);
  const tokenProgram = overrides.tokenProgram ?? TOKEN_PROGRAM_ID;
  const activeVault = overrides.activeVault ?? ctx.activeVault;
  const mint = overrides.mint ?? ctx.mint;
  return new TransactionInstruction({
    programId: PROGRAM_ID,
    keys: [
      { pubkey: ctx.user.publicKey, isSigner: true, isWritable: true },
      ...
      { pubkey: ctx.position, isSigner: false, isWritable: true },
      { pubkey: ctx.userTokenAccount, isSigner: false, isWritable: true },
      { pubkey: activeVault, isSigner: false, isWritable: true },
...
export async function bootstrapMarketContext(
  ...
  await sendIx(provider, [
    upsertTemplateIx({
      ...
      oracleMaxDelaySeconds: templateConfig.oracleMaxDelaySeconds,
      oracleMaxConfidenceBps: templateConfig.oracleMaxConfidenceBps,
    }),
  ]);
