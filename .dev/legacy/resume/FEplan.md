...
- **Location**: Refactor into [`apps/web/src/features/world-cup/`](apps/web/src/features/world-cup/) as the canonical feature module.
...
Before moving code, map these existing touchpoints in [`apps/web/`](apps/web/):
...
Create [`apps/web/src/features/world-cup/`](apps/web/src/features/world-cup/) with:
...
**Source migration**: Lift UI from [`apps/web/src/.popular-global-events/world_cup/`](apps/web/src/.popular-global-events/world_cup/) into the structure above. Keep the dot-folder only as a thin re-export shim temporarily (or remove after migration) to avoid broken imports.
...
When porting from [`.popular-global-events/world_cup/`](apps/web/src/.popular-global-events/world_cup/):
...
- [`apps/web/package.json`](apps/web/package.json) — add deps only if standalone UI introduced something missing (prefer existing icon/date libs).
- [`apps/web/tsconfig.json`](apps/web/tsconfig.json) / `vite.config.ts` — add `@/features/world-cup` alias only if project already uses path aliases; otherwise use relative imports consistent with codebase.
...
pnpm --filter web typecheck   # if script exists
```
