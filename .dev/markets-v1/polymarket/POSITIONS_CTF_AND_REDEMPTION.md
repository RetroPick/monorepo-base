...
## 5. Current state
Data API not integrated; redemption flow designed only.
## 6. Target design

### 6.1 Outcome token identity

- ERC-1155 position IDs from CTF `getPositionId(collectionId, conditionId, indexSet)`.
- BFF resolves via Gamma/CLOB market metadata — never guess token IDs.

### 6.2 Position sources

| View | Source | Refresh |
|------|--------|---------|
| Open positions | Data API `/positions` | 30s poll |
| Balances | CLOB `/balances` | on trade |
| On-chain | indexer | block |

### 6.3 CTF operations

| Op | Contract | When |
|----|----------|------|
| split | Conditional Tokens | USDC/pUSD → outcome tokens |
| merge | Conditional Tokens | outcome tokens → collateral |
| redeem | Conditional Tokens | after resolution |

Gasless via relayer when authorized (EV-009).

### 6.4 Resolution states

| State | Meaning | User action |
|-------|---------|-------------|
| active | Trading open | trade |
| resolved | Outcome determined | redeem winning |
| redeemable | Claims available | redeem CTA |
| redeemed | Completed | view history |

### 6.5 Cost basis / PnL

- Realized PnL from fills (CLOB).
...
sequenceDiagram
  ...
  participant UMA as Resolution
  Note over UMA: Market resolves via UMA adapter
  B->>B: detect resolved + user winning balance
  B-->>C: push redeemable notification
  U->>C: Redeem
  C->>B: POST /markets/positions/redeem/preview
  B-->>C: redeem batch preview
  C->>C: sign relayer payload
  C->>B: POST /markets/positions/redeem/submit
  B->>RL: submit redeemPositions
  RL->>CTF: redeemPositions
  CTF-->>B: tx confirmed
  B-->>C: redeemed — pUSD credited
```
...
Indexer watches Resolution events → marks markets resolved → enables redeem preview.
