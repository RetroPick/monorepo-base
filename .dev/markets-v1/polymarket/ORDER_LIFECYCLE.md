...
Map Polymarket order states from preview through fill/cancel/reconcile.
...
CLOB V2 order schema; timeout enters reconciliation not duplicate submit.
...
stateDiagram-v2
  [*] --> Preview
  Preview --> Signed: user_approves
  Signed --> Submitted: BFF_relays
  Submitted --> Open: accepted
  Open --> PartialFill: match
  Open --> Cancelled: user_cancel
