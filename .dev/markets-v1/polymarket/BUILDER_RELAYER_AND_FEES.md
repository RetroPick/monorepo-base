...
## 5. Current state
Builder codes documented upstream; BFF not yet attaching `builder` field on orders.
## 6. Target design

### 6.1 Builder program (EV-010)

- **Builder code**: bytes32 on signed order — replaces V1 `POLY_BUILDER_*` HMAC headers.
- Obtain from polymarket.com → Settings → Builders.
- BFF injects builder code during order assembly; clients display fee disclosure only.
- Volume attribution on leaderboard (24h lag).

### 6.2 Relayer (EV-009)

Gas-free operations for authorized builders:
...
Auth: Builder API keys OR Relayer API keys (`RELAYER_API_KEY` + `RELAYER_API_KEY_ADDRESS`).

Rate limit: **25 submits / minute** (EV-011).

### 6.3 Fee model V2 (EV-001)

- Fees **not** embedded in signed order (`feeRateBps` removed).
- Operator-set at match time; query via `getClobMarketInfo()` / fee endpoints.
- Account tier based on trailing 30-day volume.

### 6.4 RetroPick configuration

| Secret | Storage | Rotation |
|--------|---------|----------|
| builder_code | env / secret manager | quarterly |
| builder_api_key | secret manager | on compromise |
| relayer keys | per-user encrypted | user rotation |

### 6.5 User disclosure

UI MUST show: effective fee rate estimate, builder attribution notice, gasless relayer ops labeled "network fee covered by Polymarket".
...
Order path: BFF sets `builder` bytes32 from config → included in EIP-712 → credited on match.
Relayer path: BFF batches approvals/CTF ops → POST relayer `/submit` → poll tx id.
