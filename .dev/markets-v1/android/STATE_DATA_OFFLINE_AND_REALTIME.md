# STATE DATA OFFLINE AND REALTIME
...
### Room database (`MarketsDatabase`)

| Entity | Purpose | TTL / eviction |
|--------|---------|----------------|
| `EventEntity` | Catalog events | LRU + server version |
| `MarketEntity` | Market metadata | Tied to event |
| `OrderBookSnapshotEntity` | Book levels | Replace on WS snapshot |
| `CandleEntity` | Price history segments | Paginated |
| `OrderEntity` | User orders projection | Until terminal + retention |
| `FillEntity` | Fill history | Paginated |
| `PositionEntity` | Portfolio projection | Refresh on reconcile |
| `CacheMetaEntity` | Freshness metadata | Per resource key |

All entities include: `serverVersion`, `fetchedAt`, `sequence`, `freshnessStatus`.

### DataStore usage

| File | Contents | Encryption |
|------|----------|------------|
| `user_prefs.pb` | Theme, haptics, alert defaults | Encrypted preferred |
| `session.pb` | Opaque session token ref | Encrypted |
| `onboarding.pb` | Terms acceptance version | Plain |
| `ws_cursor.pb` | Last WS sequence per channel | Plain |

Use `EncryptedSharedPreferences` master key via Android Keystore for sensitive prefs.

### Freshness model

```kotlin
enum class FreshnessStatus { LIVE, STALE, OFFLINE, UNKNOWN }

data class FreshnessLabel(
...
UI copy: "Live", "Updated 12s ago", "Offline snapshot — trading disabled".

### Stale guards for orders (critical)

| Action | Guard |
|--------|-------|
| Show preview button | Eligibility fresh + market snapshot < 30s OR user acknowledges stale |
| Enable sign | Preview hash valid + preview age < TTL from server |
| Submit | Same preview hash; mutex lock |
| Cancel | Order status fresh OR optimistic with reconcile |
| Background resume | Revalidate preview if `expiresAt` passed |

**Never allow sign/submit from OFFLINE freshness.**

### Realtime client (ADR-005)

1. Connect with `Last-Event-ID` / cursor from DataStore.
2. Apply ordered deltas to Room in transaction.
3. On gap: emit `GapDetected`; repository fetches REST snapshot.
4. Atomic replace of projection tables; resume WS.

Channels: `market.{id}.book`, `user.orders`, `user.fills`, `user.positions`.

### Offline behavior matrix

| Data | Offline read | Offline write |
|------|--------------|---------------|
| Event feed | Yes (cached) | No |
| Market rules | Yes | No |
| Order book | Yes (labeled stale) | No |
| Watchlist | Yes (queued sync) | Queue mutation |
| Order preview | No | No |
| Submit order | No | No |
| Notification prefs | Yes | Queue |

### Repository merge strategy

```kotlin
fun observeMarket(id: MarketId): Flow<MarketDetail> =
    combine(local.observe(id), realtime.updates(id)) { local, delta ->
        reducer.merge(local, delta)
    }.onStart { emitAll(local.observe(id)) }
```

### Paging

- Discovery: `PagingSource` + remote mediator writes Room.
- Order history: keyed pagination with `nextCursor` from API.
- Invalidation on WS fill event.

### Conflict resolution

Server wins on all trading state. Local optimistic UI for cancel only with rollback on 409.

### Migration policy

- Room auto-migrations where possible; exported schemas in `schemas/`.
- Destructive migration only in devDebug.
- User data migrations tested in `MigrationTest`.

### Money types

Use `Money` value class with minor units (`Long`) — never `Double` for prices/quantities.

### Degraded mode

When `/markets/capabilities` reports `orderSubmissionEnabled: false`:

- Catalog remains available.
- Order ticket shows incident banner.
- Existing orders viewable; cancel if capability allows.

See [.dev/ANDROID_MARKETS.md](../../ANDROID_MARKETS.md) §9 for offline rules.
...
| Client cache | Browser + SWR | Room + DataStore | Android offline labels required |
...
- Stale guards block sign when OFFLINE.
- WS gap recovery matches ADR-005.
- Money types never use Double.
- Room migrations exported.
...
### A.1 state-offline — Overview

**Overview** for `state-offline`.
...
- Deep dive 1 for state-offline: Overview implementation checklist item 1.
- Deep dive 1 for state-offline: Overview implementation checklist item 2.
- Deep dive 1 for state-offline: Overview implementation checklist item 3.
- Deep dive 1 for state-offline: Overview implementation checklist item 4.
- Deep dive 1 for state-offline: Overview implementation checklist item 5.

### A.2 state-offline — Module ownership

**Module ownership** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 2 for state-offline: Module ownership implementation checklist item 1.
- Deep dive 2 for state-offline: Module ownership implementation checklist item 2.
- Deep dive 2 for state-offline: Module ownership implementation checklist item 3.
- Deep dive 2 for state-offline: Module ownership implementation checklist item 4.
- Deep dive 2 for state-offline: Module ownership implementation checklist item 5.

### A.3 state-offline — API mapping

**API mapping** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 3 for state-offline: API mapping implementation checklist item 1.
- Deep dive 3 for state-offline: API mapping implementation checklist item 2.
- Deep dive 3 for state-offline: API mapping implementation checklist item 3.
- Deep dive 3 for state-offline: API mapping implementation checklist item 4.
- Deep dive 3 for state-offline: API mapping implementation checklist item 5.

### A.4 state-offline — State machine

**State machine** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 4 for state-offline: State machine implementation checklist item 1.
- Deep dive 4 for state-offline: State machine implementation checklist item 2.
- Deep dive 4 for state-offline: State machine implementation checklist item 3.
- Deep dive 4 for state-offline: State machine implementation checklist item 4.
- Deep dive 4 for state-offline: State machine implementation checklist item 5.

### A.5 state-offline — Caching

**Caching** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 5 for state-offline: Caching implementation checklist item 1.
- Deep dive 5 for state-offline: Caching implementation checklist item 2.
- Deep dive 5 for state-offline: Caching implementation checklist item 3.
- Deep dive 5 for state-offline: Caching implementation checklist item 4.
- Deep dive 5 for state-offline: Caching implementation checklist item 5.

### A.6 state-offline — Error UX

**Error UX** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 6 for state-offline: Error UX implementation checklist item 1.
- Deep dive 6 for state-offline: Error UX implementation checklist item 2.
- Deep dive 6 for state-offline: Error UX implementation checklist item 3.
- Deep dive 6 for state-offline: Error UX implementation checklist item 4.
- Deep dive 6 for state-offline: Error UX implementation checklist item 5.

### A.7 state-offline — Testing hooks

**Testing hooks** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 7 for state-offline: Testing hooks implementation checklist item 1.
- Deep dive 7 for state-offline: Testing hooks implementation checklist item 2.
- Deep dive 7 for state-offline: Testing hooks implementation checklist item 3.
- Deep dive 7 for state-offline: Testing hooks implementation checklist item 4.
- Deep dive 7 for state-offline: Testing hooks implementation checklist item 5.

### A.8 state-offline — Rollout flags

**Rollout flags** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 8 for state-offline: Rollout flags implementation checklist item 1.
- Deep dive 8 for state-offline: Rollout flags implementation checklist item 2.
- Deep dive 8 for state-offline: Rollout flags implementation checklist item 3.
- Deep dive 8 for state-offline: Rollout flags implementation checklist item 4.
- Deep dive 8 for state-offline: Rollout flags implementation checklist item 5.

### A.9 state-offline — Performance budget

**Performance budget** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 9 for state-offline: Performance budget implementation checklist item 1.
- Deep dive 9 for state-offline: Performance budget implementation checklist item 2.
- Deep dive 9 for state-offline: Performance budget implementation checklist item 3.
- Deep dive 9 for state-offline: Performance budget implementation checklist item 4.
- Deep dive 9 for state-offline: Performance budget implementation checklist item 5.

### A.10 state-offline — Accessibility

**Accessibility** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 10 for state-offline: Accessibility implementation checklist item 1.
- Deep dive 10 for state-offline: Accessibility implementation checklist item 2.
- Deep dive 10 for state-offline: Accessibility implementation checklist item 3.
- Deep dive 10 for state-offline: Accessibility implementation checklist item 4.
- Deep dive 10 for state-offline: Accessibility implementation checklist item 5.

### A.11 state-offline — Security controls

**Security controls** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 11 for state-offline: Security controls implementation checklist item 1.
- Deep dive 11 for state-offline: Security controls implementation checklist item 2.
- Deep dive 11 for state-offline: Security controls implementation checklist item 3.
- Deep dive 11 for state-offline: Security controls implementation checklist item 4.
- Deep dive 11 for state-offline: Security controls implementation checklist item 5.

### A.12 state-offline — Observability

**Observability** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 12 for state-offline: Observability implementation checklist item 1.
- Deep dive 12 for state-offline: Observability implementation checklist item 2.
- Deep dive 12 for state-offline: Observability implementation checklist item 3.
- Deep dive 12 for state-offline: Observability implementation checklist item 4.
- Deep dive 12 for state-offline: Observability implementation checklist item 5.

### A.13 state-offline — Migration notes

**Migration notes** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 13 for state-offline: Migration notes implementation checklist item 1.
- Deep dive 13 for state-offline: Migration notes implementation checklist item 2.
- Deep dive 13 for state-offline: Migration notes implementation checklist item 3.
- Deep dive 13 for state-offline: Migration notes implementation checklist item 4.
- Deep dive 13 for state-offline: Migration notes implementation checklist item 5.

### A.14 state-offline — FAQ

**FAQ** for `state-offline`.
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 14 for state-offline: FAQ implementation checklist item 1.
- Deep dive 14 for state-offline: FAQ implementation checklist item 2.
- Deep dive 14 for state-offline: FAQ implementation checklist item 3.
- Deep dive 14 for state-offline: FAQ implementation checklist item 4.
- Deep dive 14 for state-offline: FAQ implementation checklist item 5.

### A.15 state-offline — Overview
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 15 for state-offline: Overview implementation checklist item 1.
- Deep dive 15 for state-offline: Overview implementation checklist item 2.
- Deep dive 15 for state-offline: Overview implementation checklist item 3.
- Deep dive 15 for state-offline: Overview implementation checklist item 4.
- Deep dive 15 for state-offline: Overview implementation checklist item 5.

### A.16 state-offline — Module ownership
...
- Main thread must not perform network or disk I/O.
- Feature flags read from /markets/capabilities on cold start and resume.
- Deep dive 16 for state-offline: Module ownership implementation checklist item 1.
- Deep dive 16 for state-offline: Module ownership implementation checklist item 2.
- Deep dive 16 for state-offline: Module ownership implementation checklist item 3.
- Deep dive 16 for state-offline: Module ownership implementation checklist item 4.
- Deep dive 16 for state-offline: Module ownership implementation checklist item 5.
