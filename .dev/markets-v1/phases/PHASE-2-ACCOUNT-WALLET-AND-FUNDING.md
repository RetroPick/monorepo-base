...
Phase specification for **PHASE-2**: Eligibility, wallet connect, funding/deposit foundations.

## 2. Scope

### In scope

- Tasks and deliverables assigned to PHASE-2 in [implementation-manifest.yaml](../agent-harness/implementation-manifest.yaml).
...
Eligibility, wallet connect, funding/deposit foundations.
...
- Phase ID `PHASE-2` is locked per master prompt §15.

## 9. Data and control flows

```mermaid
flowchart TB
  Prev[Prior_phase_exit_gate] --> PHASE-2
  PHASE-2 --> Next[Next_phase]
```
...
- fail-closed unsupported regions
- no raw key custody
