...
Phase specification for **PHASE-0**: Eliminate unknowns that invalidate signing, custody, or scope.

## 2. Scope

### In scope

- Tasks and deliverables assigned to PHASE-0 in [implementation-manifest.yaml](../agent-harness/implementation-manifest.yaml).
...
Eliminate unknowns that invalidate signing, custody, or scope.
...
- Phase ID `PHASE-0` is locked per master prompt §15.

## 9. Data and control flows

```mermaid
flowchart TB
  Prev[Prior_phase_exit_gate] --> PHASE-0
  PHASE-0 --> Next[Next_phase]
```
...
- no unverified contract address
