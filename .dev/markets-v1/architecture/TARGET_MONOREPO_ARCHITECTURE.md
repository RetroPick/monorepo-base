...
Target monorepo layout for Markets, PRISM boundary, and legacy quarantine.
...
- `packages/polymarket/` venue adapter (to be created).
- `apps/backend/internal/markets/` BFF owns normalization.
- `apps/web/src/products/{markets,prism,legacy}` separate product shells.
- `deploy/web-markets/` isolated deploy config.
