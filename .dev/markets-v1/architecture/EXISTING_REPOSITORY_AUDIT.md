...
Inventory repository state after R0–R3 restructure for Markets V1.
...
| Area | Path | State |
|------|------|-------|
| Web | `apps/web/` | Next.js; Markets product shell |
| Backend Markets | `apps/backend/internal/markets/` | Eligibility stub, Gamma catalog |
| Legacy | `apps/backend/internal/legacy/domain/` | Epoch domain quarantined |
| OpenAPI | `schemas/openapi/markets-v1.yaml` | eligibility, capabilities, events |
| Android | `apps/android/` | README-only |
| PRISM | `contracts/prism/` | Placeholder |
| Legacy API | `/api/v1/legacy/markets/*` | Frozen |

## 6. Target design

Single source of truth for what exists vs what Markets V1 must build.
