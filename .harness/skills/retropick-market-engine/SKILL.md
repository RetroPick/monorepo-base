...
Project-specific guardrails for RetroPick: **epoch-gated** prediction markets, **UUPS `MarketEngineDispatcher`**, Go **apps/backend** (API + indexer + keeper + realtime), Next.js **fe-v1**, and Foundry in **`package/prediction-v2`**.
...
- Kanban task scoping: cite `package/prediction-v2/currentSmartContract.md` and `apps/backend` docs under `.dev/backend/` (deep reference; runtime code lives in `apps/backend/`).

## Invariants (do not violate)

1. **Epoch lifecycle** is authoritative on-chain; off-chain mirrors it — never “invent” states the engine cannot emit.
2. **Submodule** `package/prediction-v2` must stay initialized; Foundry tests required for Solidity edits.
