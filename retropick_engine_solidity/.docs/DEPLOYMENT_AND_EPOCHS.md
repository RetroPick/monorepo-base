...
|------|-----------------|------------|
| `genesisStartRolling` | `test_gas_genesis_start_rolling` | **197247** |
| `genesisLockRolling` | `test_gas_genesis_lock_rolling` | **222832** |
| `executeRollingRound` (steady) | `test_gas_rolling_execute_one_tick` | **372966** |

One-time bootstrap (genesis start + lock) ≈ **420079** gas (`197247 + 222832`). Steady-state **execute** is **~373k** gas—same order as **manual Direction** open + lock + resolve (**375944** gas combined from [`EpochGasTest`](test/gas/EpochGas.t.sol)), but rolling uses **one L2 transaction per interval** instead of **three**, which typically **dominates savings** on rollups (L1 data fee × 3 → × 1 for keeper txs).
...
|-------|------|---------------------|-------------------|------------|----------------------------------------|
| **Deploy** | Stack deploy + `initializeConfig` | Once per chain / environment (`PythAdapter`, `MarketEngine`, config) | 2–3 | *not in snapshot—measure (`forge script --dry-run`)* | *e.g. ~$3.75 exec if `G=25M`—see [Deployment cost in USD](#deployment-cost-in-usd-how-to-estimate)* |
...
| **Epoch round (Manual)** | `openEpoch` (cold) | First step each **epoch**; `epochId` must be `lastResolved + 1` (starts at `1`) | 1 / epoch | 191220 | ~$0.0287 |
...
| **Subtotal** | Keeper **Threshold / RangeClose** round | `open` + `lock` + `resolve` per epoch | **3** / epoch | **331282** | **~$0.0497** / epoch |
| **Subtotal** | Keeper **Direction** round (Manual) | `open` + `lock` + `resolve` per epoch | **3** / epoch | **375944** | **~$0.0564** / epoch |
| **Rolling (Direction)** | `genesisStartRolling` + `genesisLockRolling` | Once per rolling **session** (or after `resetRollingLifecycle`) | **2** (one-time) | **420079** (sum) | **~$0.063** (one-time, illustration) |
| **Rolling (Direction)** | `executeRollingRound` | Steady state: resolve `k-1`, lock `k`, open `k+1` | **1** / interval | **372966** | **~$0.0559** / tick |
| **User** | `claim` | Per **user** × **epoch** (after resolve / void / cancel payout path) | 1 / claim | 44068 | ~$0.0066 / claim |
| **Scale (Manual)** | Monthly keeper (example) | `E` epochs × `T` templates × **3** txs | `3 × E × T` | Threshold ≈ `E × T × 331282` | `E × T × ~$0.0497` exec-only |
| **Scale (Rolling)** | Monthly keeper (steady) | `E` intervals × `T` templates × **1** tx (+ amortize 2 genesis txs / session) | `E × T` (+2) | ≈ `E × T × 372966` | `E × T × ~$0.0559` exec-only |
...
- **Pyth:** `lock` / `resolve` (and rolling keeper calls) revert if the on-chain feed is stale; budget **oracle update** txs separately from the gas column.
...
| Keeper txs (Manual) | `200 × 3 = 600` | Ignores optional `cancelEpoch` or extra Pyth update txs. |
| Keeper txs (Rolling steady) | `200 × 1 = 200` | Plus **2** genesis txs per bootstrap session (~**420079** gas total). |
| Gas / epoch (Threshold / RangeClose, Manual) | `331282` | [`.gas-snapshot`](.gas-snapshot) sum: `191220 + 10203 + 129859`. |
| Gas / epoch (Direction, Manual) | `375944` | Sum: `191220 + 56034 + 128690`. |
| Gas / tick (Direction, Rolling steady) | `372966` | `test_gas_rolling_execute_one_tick`. |
| **Total gas / month (Threshold, Manual)** | `66,256,400` | `200 × 331282`. |
| **Total gas / month (Direction, Manual)** | `75,188,800` | `200 × 375944`. |
| **Total gas / month (Direction, Rolling steady)** | `74,593,200` | `200 × 372966` (exclude one-time genesis). |
...
**L1 data fees (rollups):** illustrative **$0.03** per keeper tx: **Manual** `600 × $0.03` = **~$18.00**; **Rolling steady** `200 × $0.03` = **~$6.00** (add **2 × $0.03** if counting genesis txs). Totals **before** Pyth pull/update costs.

**Pyth:** if your flow includes separate `updatePriceFeeds` (or similar) so `getPriceNoOlderThan` succeeds at `lock`/`resolve`, add those transaction counts and fees to the monthly budget.
...
**Reality check:** every example below is a **price** market: you need a live, non-stale feed (for example Pyth) on your chain for that asset. Narrative is marketing; settlement is always oracle math.
...
1. `new PythAdapter(IPyth pyth)`
2. `new MarketEngine()`
3. `initializeConfig(...)` on the new engine **from the same wallet** that deployed `MarketEngine`.

The deployer is fixed in the engine constructor as `deployer = msg.sender` ([`MarketEngine`](src/MarketEngine.sol)); only that address may call `initializeConfig` once. If deployment and initialization are split across separate public transactions, a frontrunner could initialize first—**prefer one scripted bundle**, a private mempool, or deploy+init atomically (see [AUDIT_SOLIDITY.md](./AUDIT_SOLIDITY.md)).
...
Let `G` = total gas used for the transaction (deploy script may be ~2–4 txs: `PythAdapter`, `MarketEngine`, `initializeConfig`; sum them).
...
### Pyth (operational cost beyond engine gas)

[`PythAdapter`](src/adapters/PythAdapter.sol) reads the Pyth contract with **`getPriceNoOlderThan`** (view). If the on-chain update is **stale** relative to `oracleConfig.maxDelaySeconds` from `initializeConfig`, **`lockEpoch` / `resolveEpoch` revert** until the feed is fresh.

On many chains, keepers pay for **price updates** (`updatePriceFeeds` or similar) **in addition to** engine execution gas. Budget that separately from the numbers in `.gas-snapshot`.
...
|------|----------------------|----------------|
| `openEpoch` (cold) | 191220 | `EpochGasTest:test_gas_openEpoch_cold` |
| `lockEpoch` (threshold) | 10203 | `test_gas_lockEpoch_threshold` |
| `lockEpoch` (direction) | 56034 | `test_gas_lockEpoch_direction` |
| `resolveEpoch` (threshold) | 129859 | `test_gas_resolveEpoch_threshold` |
| `resolveEpoch` (direction) | 128690 | `test_gas_resolveEpoch_direction` |
| `claim` | 44068 | `test_gas_claim_afterResolve` |
| `genesisStartRolling` | 197247 | `MarketEngineRollingTest:test_gas_genesis_start_rolling` |
| `genesisLockRolling` | 222832 | `MarketEngineRollingTest:test_gas_genesis_lock_rolling` |
| `executeRollingRound` (steady) | 372966 | `test_gas_rolling_execute_one_tick` |
...
Add Pyth update costs if applicable.
...
|------|-------------------------|------|
| **Manual — Threshold / RangeClose** | `191220 + 10203 + 129859` | **331282** |
| **Manual — Direction** | `191220 + 56034 + 128690` | **375944** |
| **Rolling — Direction (steady)** | `executeRollingRound` | **372966** |

**Illustrative monthly keeper cost, one template (replace inputs)**

- Intervals per month: `E = 30` (e.g. one per day).
- **Threshold Manual:** `G_month ≈ E × 331282 ≈ 9.94M` gas (execution only).
...
**Pyth**: Oracle-update transactions (if required so `getPriceNoOlderThan` succeeds) are billed the same way—add their gas × price and any Pyth-specific charges.
...
**Theoretical upper bound:** the last openable epoch id is **`2^64 - 1`**. After that epoch is resolved, `activeEpochId + 1` would be **`2^64`**, which **overflow-reverts** in Solidity 0.8, so **no further epoch can be opened** on that template without a new engine deployment or upgraded logic. This is a numeric identifier limit, not a practical product ceiling.
...
1. **Deploy stack** (once per chain/environment): `PythAdapter` + `MarketEngine` + `initializeConfig` with your `stakeToken`, oracle adapter, `admin`, `treasury`, `workerAuthority`, fee caps, `maxOutcomes` (≤ 8), and oracle limits. See [README.md](./README.md) env table and [`Deploy.s.sol`](script/Deploy.s.sol).
...
3. **Admin: `upsertTemplate(UpsertTemplateParams)`**
   ...
   - `oracleFeedId`: Pyth price id (must be non-zero).
...
|---|----------------------------------|---------------------------|
| Oracle | Chainlink `latestRoundData`, monotonic `roundId` | Pyth (normalized) via `IPriceOracle` |
...
- **Manual** (3 txs / interval): **375944** gas / epoch × `8640` ≈ **3.25×10⁹** execution gas → **~$487** execution-only at 0.05 gwei / $3k ETH (before L1).
- **Rolling** (1 tx / interval in steady state): **372966** gas × `8640` ≈ **3.22×10⁹** execution gas → **~$483** execution-only—similar **per-interval** execution to Manual’s **sum of three txs**, but **one keeper transaction** per tick like Pancake.

Versus a **200k** gas Pancake `executeRound` × `8640` (**1.73×10⁹** gas, **~$259** exec-only), RetroPick Direction paths use **~1.9×** the execution gas per interval in these snapshots (more accounting, Pyth path, pro-rata settlement)—**measure your build**.
