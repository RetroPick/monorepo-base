...
library MarketTypes {
    ...
    function validateCheckpointAPublishTime(Epoch storage e, uint64 publishTime) internal view returns (bool) {
        return publishTime >= e.timing.lockAt;
    }

    function validateCheckpointBPublishTime(Epoch storage e, uint64 publishTime) internal view returns (bool) {
        if (publishTime < e.timing.resolveAt) return false;
