...
abstract contract MarketEngineState {
    ...
    error InvalidYieldRoute(bytes32 templateId);
    ...
    event TemplateYieldRouteLocked(
        bytes32 indexed templateId, bytes32 indexed routeId, address indexed strategy, uint8 strategyKind
    );
    ...
    function _applyResolveAccounting(
        ...
        if (outputs.settlementFeeTotal > 0) {
            ...
            emit ProtocolFeeAccrued(
                ...
                e.settlementFeeBps,
                e.totalPool - e.winningPoolTotal,
                e.winningPoolTotal
            );
        }
