...
Deep reference for **rolling execution mode** — the Pancake-style pipeline where one keeper transaction advances **resolve → lock → open** on a fixed interval. This complements [`currentSmartContract.md`](currentSmartContract.md). Operator incidents: [`.operator/.runbook.md`](.operator/.runbook.md).
...
- Rolling ticks are **keeper-driven**; compromise of `workerAuthority` or keeper keys affects timing, not oracle truth (resolution still uses configured feeds).
- **Halt / pause / reset** paths are admin-gated; use them when phase checks fail or oracle stalls (see §5 and `currentSmartContract.md` §10).
- Pre-launch checklist: [`.docs/deployment/ProductionChecklist.md`](.docs/deployment/ProductionChecklist.md).
