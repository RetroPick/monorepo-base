...
Usage:
  ...
  python3 scripts/estimate_deploy_and_epoch_costs.py --rpc-url https://mainnet.base.org --json
  python3 scripts/estimate_deploy_and_epoch_costs.py --no-deploy-sim --json

Env (optional):
  RPC_URL / BASE_SEPOLIA_RPC_URL / BASE_RPC_URL
  FORGE_SCRIPT_TIMEOUT         Seconds for each `forge script` dry-run (default 600)
  ETH_PRICE_USD
...
CLI:
  ...
  --deploy-fork-rpc-url URL  RPC for deploy dry-runs only (default: Sepolia when --rpc-url is mainnet)
  --forge-timeout SECONDS    Max wait per forge dry-run (default 600)
"""
...
DEFAULT_FORGE_TIMEOUT = 600
DEFAULT_CAST_CMD_TIMEOUT = 30
...
def run_cmd(
    ...
    *,
    ...
    timeout: int = DEFAULT_CAST_CMD_TIMEOUT,
) -> str:
    try:
        out = subprocess.run(
            cmd,
...
def default_forge_timeout() -> int:
    raw = os.environ.get("FORGE_SCRIPT_TIMEOUT", "").strip()
    if raw:
        return max(1, int(raw))
    return DEFAULT_FORGE_TIMEOUT


def resolve_deploy_fork_rpc(
    pricing_rpc: str,
    deploy_fork_rpc: str | None,
    pricing_chain_id: int,
) -> tuple[str, str | None]:
    """Pick RPC for `forge script` forks. Returns (fork_rpc, auto_redirect_reason)."""
    if deploy_fork_rpc:
        return deploy_fork_rpc.strip(), None
    if pricing_chain_id == BASE_MAINNET_CHAIN_ID:
        return (
            DEFAULT_BASE_SEPOLIA_RPC,
            "pricing RPC is Base mainnet; deploy simulation uses Base Sepolia fork "
            f"({DEFAULT_BASE_SEPOLIA_RPC})",
        )
    return pricing_rpc, None
...
def simulate_deploy(
    *,
    ...
    if not shutil.which("forge"):
        ...
        return _fallback_deploy_estimate(
            label=label,
            fallback_gas_units=fallback_gas_units,
    ...
    info(
        f"Running forge dry-run: {label} "
        f"(fork RPC: {fork_rpc_url}, chain id {fork_chain_id}, timeout: {forge_timeout}s)"
    )
    try:
        proc = subprocess.run(
            cmd,
        ...
        notes.append(f"forge dry-run timed out after {forge_timeout}s")
        return _fallback_deploy_estimate(
            label=label,
            fallback_gas_units=fallback_gas_units,
            gas_price_wei=gas_price_wei,
    ...
    fee_mode = "execution_only"
    try:
        l1_data_fee_wei = estimate_l1_fees_for_transactions(
            pricing_rpc_url, pricing_chain_id, txs, gas_price_wei
        )
...
def main() -> int:
    ...
    p.add_argument(
        "--rpc-url",
        default=env_rpc_url(),
        help="RPC for gas price, chain id, and L1 fee pricing (may be Base mainnet)",
    )
    p.add_argument(
        "--deploy-fork-rpc-url",
        default=None,
        help="RPC for forge deploy dry-runs only (default: Sepolia when --rpc-url is Base mainnet)",
    )
    p.add_argument(
        "--forge-timeout",
        type=int,
        default=default_forge_timeout(),
        help="Max seconds per forge script dry-run (default 600; env FORGE_SCRIPT_TIMEOUT)",
    )
    p.add_argument(
        "--allow-mainnet-deploy-sim",
        action="store_true",
        help="Suppress warning when --deploy-fork-rpc-url points at Base mainnet",
    )
    ...
    pricing_rpc = args.rpc_url
    try:
        chain_id = cast_chain_id(pricing_rpc)
        chain_id_source = "rpc"
    except Exception as e:
        warn(f"could not read chain id from RPC ({e}); assuming Base Sepolia chain id {BASE_SEPOLIA_CHAIN_ID}")

    deploy_fork_rpc, fork_redirect_reason = resolve_deploy_fork_rpc(
        pricing_rpc, args.deploy_fork_rpc_url, chain_id
    )
    if fork_redirect_reason:
        warn(fork_redirect_reason)

    if (
        not args.allow_mainnet_deploy_sim
        and args.deploy_fork_rpc_url
        and deploy_fork_rpc.strip().lower() != pricing_rpc.strip().lower()
    ):
        try:
            fork_cid = cast_chain_id(deploy_fork_rpc)
            if fork_cid == BASE_MAINNET_CHAIN_ID:
                warn(
                    "deploy fork RPC is Base mainnet; forge dry-runs can take a long time "
                    "(use --allow-mainnet-deploy-sim to silence)"
                )
        except Exception:
            pass

    fork_chain_id = BASE_SEPOLIA_CHAIN_ID
    fork_chain_id_source = "default"
    try:
        fork_chain_id = cast_chain_id(deploy_fork_rpc)
        fork_chain_id_source = "rpc"
    except Exception as e:
        warn(
            f"could not read chain id from deploy fork RPC ({e}); "
            f"assuming Base Sepolia chain id {BASE_SEPOLIA_CHAIN_ID}"
        )

    deploy_sim_skipped_reason: str | None = None
    if args.no_deploy_sim:
        deploy_sim_skipped_reason = "disabled via --no-deploy-sim"
    elif fork_redirect_reason:
        deploy_sim_skipped_reason = fork_redirect_reason
    ...
    if args.gas_price_gwei is not None:
        ...
        try:
            gas_price_wei = cast_gas_price_wei(pricing_rpc)
    ...
    if args.no_deploy_sim:
        ...
        deploy_estimates["deploy_production"] = simulate_deploy(
            ...
            env_overrides=default_production_env(fork_chain_id),
            fork_rpc_url=deploy_fork_rpc,
            fork_chain_id=fork_chain_id,
            pricing_rpc_url=pricing_rpc,
            pricing_chain_id=chain_id,
            gas_price_wei=gas_price_wei,
        ...
        deploy_estimates["deploy_testnet_with_faucet"] = simulate_deploy(
            ...
            env_overrides=default_testnet_env(fork_chain_id),
            fork_rpc_url=deploy_fork_rpc,
            fork_chain_id=fork_chain_id,
            pricing_rpc_url=pricing_rpc,
            pricing_chain_id=chain_id,
    ...
    if args.json:
        payload = {
            ...
            "pricing_rpc_url": pricing_rpc,
            "deploy_fork_rpc_url": deploy_fork_rpc,
            "rpc_url": pricing_rpc,
            "chain_id": chain_id,
            "chain_id_source": chain_id_source,
            "deploy_fork_chain_id": fork_chain_id,
            "deploy_fork_chain_id_source": fork_chain_id_source,
            "forge_timeout_seconds": args.forge_timeout,
            ...
            "deploy_simulation_skipped_reason": deploy_sim_skipped_reason,
            "defaults": {
    ...
    print(
        term.dim(
            f"Pricing RPC: {pricing_rpc}  |  chain id: {chain_id} ({chain_id_source})  |  "
            f"Deploy fork RPC: {deploy_fork_rpc}  |  fork chain id: {fork_chain_id} ({fork_chain_id_source})  |  "
