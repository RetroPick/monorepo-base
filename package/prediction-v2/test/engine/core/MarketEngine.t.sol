...
contract MarketEngineTest is MarketEngineBase {
    ...
    function test_upsertTemplate_revertsWhenSettlementFeeExceedsProtocolCap() public {
        vm.startPrank(admin);
        bytes4 err = bytes4(keccak256("InvalidFeeBps()"));
        vm.expectRevert(err);
        engine.upsertTemplate(_templateWithSettlementFee("fee-high", 201));
        vm.stopPrank();
    }

    function test_protocolFeeAccrued_chargesLosingStakePoolOnly() public {
        vm.startPrank(admin);
        engine.upsertTemplate(_templateWithSettlementFee("fee-event", 100));
        bytes32 tid = _tid("fee-event");
        engine.initializeMarket(tid);
        vm.stopPrank();
        ...
        vm.warp(t0 + 150);
        token.mint(address(this), 2000e18);
        token.approve(address(engine), type(uint256).max);
        engine.depositToSide(tid, 1, 0, 1000e18);
        ...
        emit MarketEngineState.ProtocolFeeAccrued(tid, 1, address(token), 10e18, 100, 1000e18, 1000e18);

        vm.prank(worker);
        engine.resolveEpoch(tid, 1);

        assertEq(engine.epochs(tid, 1).settlementFeeTotal, 10e18);
    }

    function _templateWithSettlementFee(string memory slug, uint16 settlementFeeBps)
        internal
        view
        returns (MarketEngine.UpsertTemplateParams memory p)
    {
        p = _defaultTemplate(slug);
        p.settlementFeeBps = settlementFeeBps;
    }
}
