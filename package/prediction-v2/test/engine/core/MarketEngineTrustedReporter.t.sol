...
contract MarketEngineTrustedReporterTest is MarketEngineBase {
    ...
    function test_tro_rangeClose_manual_resolve() public {
        ...
        vm.startPrank(user);
        token.approve(address(engine), 500e18);
        engine.depositToSide(tidR, 1, 1, 500e18);
        vm.stopPrank();
    ...
    function test_RevertWhen_corridorUsesChainlinkScalarFeed() public {
        MarketEngine.UpsertTemplateParams memory p = _corridorTemplate("bad-corridor-chainlink", address(tro));
        p.templateOracleKind = MarketTypes.OracleKind.Chainlink;
        p.oracleFeedId = feed;
        p.eventOracle = address(0);
    ...
    function test_RevertWhen_cascadeUsesChainlinkScalarFeed() public {
        MarketEngine.UpsertTemplateParams memory p = _cascadeTemplate("bad-cascade-chainlink", address(tro), false);
        p.templateOracleKind = MarketTypes.OracleKind.Chainlink;
        p.oracleFeedId = feed;
        p.eventOracle = address(0);
