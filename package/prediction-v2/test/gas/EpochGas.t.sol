...
contract EpochGasTest is MarketEngineBase {
    function _openGasEpoch(string memory slug, uint64 epochId, uint64 t0) internal returns (bytes32 tid) {
        vm.startPrank(admin);
        engine.upsertTemplate(_defaultTemplate(slug));
        tid = _tid(slug);
        engine.initializeMarket(tid);
        vm.stopPrank();
        vm.warp(t0);
        vm.prank(worker);
        engine.openEpoch(tid, epochId, t0, t0 + 100, t0 + 200);
    }
    ...
    function test_gas_depositToSide_firstPosition() public {
        vm.pauseGasMetering();
        bytes32 tid = _openGasEpoch("g-dep-first", 1, 50_000);
    ...
    function test_gas_depositToSide_existingPosition() public {
        vm.pauseGasMetering();
        bytes32 tid = _openGasEpoch("g-dep-existing", 1, 51_000);
    ...
    function test_gas_switchSide_fullBinary() public {
        vm.pauseGasMetering();
        bytes32 tid = _openGasEpoch("g-switch", 1, 52_000);
        token.mint(address(this), 1e18);
        token.approve(address(engine), type(uint256).max);
        engine.depositToSide(tid, 1, 0, 1e18);
        vm.resumeGasMetering();
        engine.switchSide(tid, 1, 0, 1, 1e18);
    }

    function test_gas_claimMany_twoEpochs() public {
        vm.pauseGasMetering();
        vm.startPrank(admin);
        engine.upsertTemplate(_defaultTemplate("g-claim-many"));
        bytes32 tid = _tid("g-claim-many");
        engine.initializeMarket(tid);
        vm.stopPrank();
        ...
        uint64 t0 = 53_000;
        for (uint64 epochId = 1; epochId <= 2; epochId++) {
            vm.warp(t0);
            vm.prank(worker);
            engine.openEpoch(tid, epochId, t0, t0 + 100, t0 + 200);
            engine.depositToSide(tid, epochId, 0, 1e18);
            vm.warp(t0 + 101);
            oracle.set(feed, 150e8, uint64(t0 + 101), 0);
            vm.prank(worker);
            engine.lockEpoch(tid, epochId);
            vm.warp(t0 + 201);
            oracle.set(feed, 150e8, uint64(t0 + 201), 0);
            vm.prank(worker);
            engine.resolveEpoch(tid, epochId);
            t0 += 1_000;
        }
