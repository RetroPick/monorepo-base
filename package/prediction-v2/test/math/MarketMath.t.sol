...
contract MarketMathTest is Test {
    ...
    function test_claim_entitlement_uses_resolved_claim_liability_with_yield() public pure {
        MT.Epoch memory epoch;
        epoch.winningOutcomeMask = 1;
        epoch.outcomeCount = 2;
        epoch.totalPool = 2000;
        epoch.outcomePools = [uint256(1000), 1000, 0, 0, 0, 0, 0, 0];
        epoch.settlementFeeBps = 100;
        epoch.feeOnLosingPool = true;
        epoch.winningPoolTotal = 1000;
        epoch.claimLiabilityTotal = 2332;

        uint256[8] memory stakes;
        stakes[0] = 1000;

        uint256 ent = MarketMath.computeTotalUserEntitlementResolved(epoch, stakes, 100, true);
        assertEq(ent, 2332);
    }
}
