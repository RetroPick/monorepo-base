...
Full personas: `.harness/agents/<slug>.agent.md`. Legacy agents (MarketEngine/epoch era) are preserved under [`archive/harness/agents/`](archive/harness/agents/) — see [`.harness/agents/README.md`](.harness/agents/README.md).
...
Legacy agent personas: `archive/harness/agents/` (reference only).
...
only; do not replace RetroPick's Go Markets BFF, Postgres projection model, or Next.js Markets web with external pool-based MarketEngine patterns.
...
- After ADR-R5, do not redo the Markets-only backend conversion or reintroduce epoch runtime paths (`cmd/api`, `apps/fe-v1`, `apps/ops-web`, indexer/keeper/funding workers); finish cleanup via `archive/`, doc rewrites, and `scripts/check-active-legacy-refs.sh`.
...
- RetroPick Go module root is `apps/backend/` (no repo-root `go.mod`). Live backend is Markets-only (ADR-R4/R5): `cmd/markets-api`, `markets-seed`, `migrator`, `healthcheck`; epoch libraries and SQL live under `archive/apps/backend/`. Live migrations are `000001`–`000010`; epoch-era databases must be recreated (`docker compose down -v`), not dual-upgraded. Run BFF with `go -C apps/backend run ./cmd/markets-api`. Local Postgres via compose uses port `:5433`. Root `docker-compose.yml` is a Markets alias; prefer seeded dev via `pnpm dev:markets-stack` / `docker-compose.markets-dev.yml`.
- CI runs `scripts/check-active-legacy-refs.sh` to block epoch runtime refs in the active tree. Legacy harness agents live in `archive/harness/agents/`; route Markets V1 work only to `rp-*` personas (see `.harness/agents/README.md`).
